...
def upgrade():
    conn = op.get_bind()
    conn.execute("CREATE UNIQUE INDEX ix_username_insensitive_unique ON public.user (LOWER(username));")
    conn.execute("CREATE UNIQUE INDEX ix_email_insensitive_unique ON public.user (LOWER(email));")
