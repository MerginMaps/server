...
def test_download_diff_file(client, diff_project):
    ...
    with open(downloaded_file, "wb") as f:
        f.write(resp.data)
    file_change = pv_2.changes.filter_by(change=PushChangeType.UPDATE_DIFF.value).first()
...
def test_push_no_diff_finish(client):
    ...
    file_meta = latest_version.changes.filter(FileHistory.change == PushChangeType.UPDATE_DIFF.value).first()
    ...
    # check diff file was generated by server, and it is in file history
    latest_version = upload.project.get_latest_version()
    # FIXME should not this be update_diff if diff was calculated by server?
    file_meta = latest_version.changes.filter(FileHistory.change == PushChangeType.UPDATE.value).first()
    assert file_meta.diff_file is None
...
def test_clone_project(client, data, username, expected):
    ...
    if expected == 200:
        ...
        assert not any(item for item in changes if item.change == PushChangeType.DELETE.value)
        assert not [item for item in changes if item.change == PushChangeType.UPDATE.value]
        assert pv.device_id == json_headers["X-Device-Id"]
        # cleanup
